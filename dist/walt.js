(function(global, factory) {
    ...
    token(
      ...
      if (parsers.length > 1) {
        parsers = parsers.filter(
          parser => (parser.strict ? parser.leaf : true)
        );
        if (parsers.length > 1) {
  ...
  function generateType(node) {
    ...
    if (right != null) {
      walker({
        [Syntax.Type]: ({ value: typeValue }) =>
          params.push(getType$1(typeValue)),
      })(left);
    }
    ...
    walker({
      [Syntax.Type]: ({ value: typeValue }) => {
        result =
          typeValue && typeValue !== "void" ? getType$1(typeValue) : null;
      },
    })(right || left);
  ...
  const findTableIndex = (ctx, functionIndex) => {
    return ctx.Program.Element.findIndex(
      n => n.functionIndex === functionIndex
    );
  };

  const findGlobalIndex = (ctx, { value }) =>
  ...
  const patchStringSubscript = (ctx, metaType, params) => {
    if (
      ...
      params[1].Type === Syntax.StringLiteral
    ) {
      ...
      return [
        params[0],
        ctx.makeNode(
          { value: absoluteByteOffset, type: "i32" },
          Syntax.Constant
        ),
      ];
    }
  ...
  const isPunctuatorAndNotBracket = t =>
  ...
  const generateDeclaration = (node, parent = { code: [], locals: [] }) => {
    ...
    if (initNode) {
      ...
      return [
        ...generateExpression(
          _extends({}, initNode, { type: node.type }),
          parent
        ),
        { kind: def.SetLocal, params: [metaIndex.payload] },
      ];
    }
  ...
  const generateArraySubscript = (node, parent) => {
    ...
    invariant_1(
      metaType,
      `Cannot generate subscript on an non-indexable node ${JSON.stringify(
        node
      )}`
    );
    const block = node.params.map(mapSyntax(parent)).reduce(mergeBlock, []);
    ...
    if (metaType.type === TYPE_ARRAY) {
      ...
      block.push.apply(block, [
        ...
        { kind: def.i32Const, params: [4] },
        { kind: def.i32Mul, params: [] },
      ]);
    }
  ...
  const generateLoop = (node, parent) => {
    ...
    block.push.apply(
      block,
      (node.body || []).map(mapper).reduce(mergeBlock, [])
    );
...
${msg}
  ...
  const parseArguments = ctx => {
    ...
    return mapNode({
      [Syntax.Pair]: pairNode => {
        ...
        return _extends({}, pairNode, {
          params: [
            _extends({}, identifierNode, { type: typeNode.type }),
            typeNode,
          ],
        });
      },
  ...
  //
  const hoistType = (ctx, typeNode, functionNode) => {
    const typeIndex$$1 = ctx.Program.Types.length;
    ctx.Program.Types.push({
      ...
      hoist: node => {
        ...
        ctx.Program.Types[typeIndex$$1] = generateType(node);
      },
    });
  ...
  function parseImport(ctx) {
    ...
    const node = patchTypeIndexes(
      ctx,
      _extends({}, baseNode, { params: [fields, module] })
    );
    ctx.Program.Imports.push.apply(
      ctx.Program.Imports,
      generateImportFromNode(node)
    );
  ...
  const hoistTypeMaybe = (ctx, node) => {
    ...
    if (get$1(TYPE_OBJECT, node) == null) {
      ...
      node.meta.push(typeIndex(ctx.Program.Types.length - 1));
  ...
  class Context {
    ...
    unsupported() {
      return this.syntaxError(
        "Language feature not supported",
        this.token.value
      );
    }
    ...
    push(type, value, debug = "") {
      ...
      switch (type) {
        ...
        default: {
          ...
          invariant_1(
            size,
            `Cannot write a value of size ${size}, type ${type}`
          );
        }
      }
  ...
  function write() {
    return new OutputStream()
      .push(wasmTypes_12, MAGIC, "\\0asm")
      .push(wasmTypes_12, VERSION, `version ${VERSION}`);
  }
  ...
  const writer = ({ type, label, emitter }) => ast => {
    ...
    const stream = new OutputStream().push(
      wasmTypes_9,
      type,
      label + " section"
    );
