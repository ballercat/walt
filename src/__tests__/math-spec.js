...
test('addition', t =>
  compileAndRun(`export function test() : i32 { return 2 + 2; }`).then(outputIs(t, 4))
);

test('subtraction', t =>
  compileAndRun(`export function test(): i32 { return 4 - 2; }`).then(outputIs(t, 2))
);
...
test('remainder', t =>
  compileAndRun('export function test(): i32 { return 5 % 2; }').then(outputIs(t, 1))
);
...
test('prefix increment', t =>
  compileAndRun('export function test(): i32 { let x: i32 = 2; return ++x; }').then(outputIs(t, 3))
);

test('prefix decrement', t =>
  compileAndRun('export function test(): i32 { let x: i32 = 2; return --x; }').then(outputIs(t, 1))
);

test('postfix increment', t =>
  compileAndRun(`
