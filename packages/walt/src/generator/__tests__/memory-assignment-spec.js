...
test("unary negation, arrays", t => {
  const ctx = mockFunction(
    mockContext("x[0] = ((x * 7 % 200) - 100) / 100.0"),
    {
      locals: [
        {
          value: "x",
...
test("generates correct offsets for arrays", t => {
  const ctx = mockFunction(mockContext("x[1] = 42;"), {
    locals: [
      { value: "x", type: "i32", meta: [{ type: TYPE_ARRAY, payload: "i32" }] },
    ],
  });
...
test("generates correct offsets for user-defined objects", t => {
  const ctx = mockFunction(mockContext("x['bar'] = 42;"), {
    locals: [
      {
        value: "x",
        type: "i32",
